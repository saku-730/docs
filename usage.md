...
- 左サイドバーは階層トグルを使わず、フラット表示（ネストなし）で表示します。
- 左サイドバーに表示するリンクは `ホーム / タグ一覧 / カテゴリ一覧`（英語は `Home / Tags / Categories`）の3つのみです。
...
- 左サイドバーの不要ブロックは `display: none` に加え、`height/min-height/margin/padding` も `0` にしてレイアウト占有をなくしています。
- サイドバー表示は `ホーム / タグ一覧 / カテゴリ一覧` を優先し、不要要素は「許可リンク以外を非表示」によって制御します。
- 3リンク以外の `li/div/ul` も、許可リンクを含まない場合は `display:none` とサイズ `0` を適用して占有をなくします。
